...
static rt_err_t ifx_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    if (uart->uart_init_flag != RT_TRUE)
    {
        ...
        /* Initialize UART hardware (only once) */
        result = cyhal_uart_init(uart->config->uart_obj, uart->config->tx_pin, uart->config->rx_pin, NC, NC, NULL, &uart_config);

        if (result == CY_RSLT_SUCCESS)
        {
            uart->uart_init_flag = RT_TRUE;
        }
    }

    /* Set baud rate (can be done even after initialization) */
    if (result == CY_RSLT_SUCCESS)
    {
...
void rt_hw_uart_init(void)
{
    ...
    for (index = 0; index < obj_num; index++)
    {
        ...
        uart_obj[index].uart_init_flag = RT_FALSE;  /* UART hardware not initialized yet */
